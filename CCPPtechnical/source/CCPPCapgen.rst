...
.. code-block:: console
  ...
  --source-files               - Comma-separated list of Fortran source (.F90) files
...
In addition to the autogenerated caps, the *ccpp_capgen* script also creates a database file, ``datatable.xml``. This file contains all the information required by the host model build system. The :term:`CCPP` *datafile* script ``ccpp-framework/capgen/ccpp_datafile.py`` can be invoked to query this database file as many times as needed within any host model.
...
*Listing 8.6: Here the CCPP datatable is used to create comma-separated lists of the autogenerated files (CAPGEN_FILES), filtered physics scheme files (SCHEME_FILES) for the provided SDF(s), and any necessary dependencies (DEPENDENCY_FILES).*
