...
In addition to the autogenerated caps, the *capgen* script also creates a ``datatable.xml``. This file contains all the information required by the build system, and can be queried as follows:
...
.. code-block:: console
  ...
  * ``--host-variables``.    - Required host model variable standard names
  ...
  ccpp/framework/capgen/ccpp_datafile.py --dependencies DEPENDENCIES
