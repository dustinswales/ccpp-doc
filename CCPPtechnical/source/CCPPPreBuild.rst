...
In addition to the autogenerated caps, the *ccpp_capgen* script also creates a database file,``datatable.xml``. This file contains all the information required by the host model build system. The :term:`CCPP` *datafile* script ``ccpp-framework/capgen/ccpp_datafile.py`` can be invoked as many times as needed for building the CCPP within any host model. 
