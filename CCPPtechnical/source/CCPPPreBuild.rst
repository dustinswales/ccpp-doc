...
*Listing 8.1: Options for *capgen* script.*
...
*Listing 8.1: Capgen autogenerated files.*

In addition to the autogenerated caps, the *capgen* script also creates a database file,``datatable.xml``. This file contains all the information required by the host model build system. The :term:`CCPP` *datafile* script ``ccpp-framework/capgen/ccpp_datafile.py`` can be invoked as many times as needed for building the CCPP within any host model. For example:
...
*Listing 8.2: Here the CCPP datatable is used to create comma-separated lists of the autogenerated files (CAPGEN_FILES), filtered physics scheme files (SCHEME_FILES), and any necessary dependencies (DEPENDENCIY_FILES).*
