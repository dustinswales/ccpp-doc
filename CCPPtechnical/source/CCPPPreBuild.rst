...
*Listing 8.1: Autogenerated files.

In addition to the autogenerated caps, the *capgen* script also creates a ``datatable.xml``. This data file contains all the information required by the host model build system. The :term:`CCPP` *datafile* script ``ccpp-framework/capgen/ccpp_datafile.py`` can be invoked as many times as needed for building the CCPP within any host model. For example:
